...
def load_membership():
    ...
    def find(names):
        for c in m.columns:
            if c.lower() in names:
                return c
        raise ValueError(
            f"Membership column not found: {m.columns.tolist()}"
        )
    ...
    m["Symbol"] = (
        m[sc].astype(str).str.strip().str.upper()
        .str.replace(".NS", "", regex=False)
    )
    m["valid_from"] = pd.to_datetime(
        m[fc], errors="coerce"
    ).dt.normalize()
    m["valid_to"] = pd.to_datetime(
        m[tc], errors="coerce"
    ).dt.normalize()
...
def monthly_data(daily):
    # Build completed calendar-month OHLC candles.
    ...
    # The monthly candle is only known after month-end.
    monthly["Available_Date"] = (
        monthly.index + pd.offsets.BDay(1)
    )
    return monthly
...
def main():
    ...
    criteria = pd.DataFrame({'Rule': [
        ...
        'Daily RSI(5) must be strictly below 31',
