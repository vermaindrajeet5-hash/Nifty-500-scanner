...
OUTPUT_FILE = "trial_1_nifty500_10yr_signals.xlsx"
...
def scan_symbol(
    ...
    # FINAL DAILY SIGNAL
    # --------------------------------------------------------
    # EMA20 condition requested by the user:
    # the candle must TOUCH EMA20 during the day
    # (Low <= EMA20) AND CLOSE above EMA20 (Close > EMA20).
    daily["EMA20_Touch_And_Close_Above"] = (
    ...
    signal = (
        ...
        # Daily candle touches EMA20
        # and closes above EMA20:
        & daily[
            "EMA20_Touch_And_Close_Above"
        ].fillna(False)
    ...
    # Build the result directly from the filtered DataFrame.
    # This avoids pandas "All arrays must be of the same length"
    # errors when Yahoo returns indicator columns with different
    # internal shapes.
    output = result.reset_index().copy()
    ...
    output = output.rename(
        columns={
            "Monthly_ADX_Difference": "ADX_Difference",
            "Close": "Daily_Close",
            "Low": "Daily_Low",
        }
    )

    output["Supertrend_Green"] = True
    output["EMA20_Touched"] = True
    output["MACD_Blue_Above_Red"] = True
