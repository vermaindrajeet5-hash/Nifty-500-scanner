...
def scan(s,d,m,start,end):
    ...
    d["Daily_RSI_5"]=rsi(d.Close,RP)
    d["Daily_RSI_5_SMA_14"]=d.Daily_RSI_5.rolling(RMA,min_periods=RMA).mean()
    ...
    q=d[(d.index>=start)&(d.index<=end)&d.Active&(d.Daily_RSI_5>d.Daily_RSI_5_SMA_14)&d.Touch&(d.ST=="GREEN")]
    out=[]
    for dt,r in q.iterrows():
        dt=pd.Timestamp(dt).normalize()
        if not member(s,dt,m):continue
        out.append({"Date":dt.date(),"Stock":s,"Monthly_RSI_5":round(float(r.MRSI),2),"Monthly_RSI_5_SMA_14":round(float(r.MSMA),2),"Monthly_Cross_Above":bool(r.Cross),"Previous_Month_End":pd.Timestamp(r.PM).date(),"Daily_RSI_5":round(float(r.Daily_RSI_5),2),"Daily_RSI_5_SMA_14":round(float(r.Daily_RSI_5_SMA_14),2),"Daily_Close":round(float(r.Close),2),"Daily_Low":round(float(r.Low),2),"Daily_High":round(float(r.High),2),"Daily_EMA_20":round(float(r.EMA20),2),"Daily_Supertrend":"GREEN"})
    return out
...
def main():
    ...
    cols=["Date","Stock","Monthly_RSI_5","Monthly_RSI_5_SMA_14","Monthly_Cross_Above","Previous_Month_End","Daily_RSI_5","Daily_RSI_5_SMA_14","Daily_Close","Daily_Low","Daily_High","Daily_EMA_20","Daily_Supertrend"]
