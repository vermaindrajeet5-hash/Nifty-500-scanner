...
def find_five_condition_events(data, symbol, membership):
    ...
    for date in data.index[five]:
        if not is_member_on_date(membership, symbol, date):
            continue

        ret = add_forward_returns(data, date)

        events.append({
            "Stock": symbol.upper(),
            "Signal_Date": pd.Timestamp(date).strftime("%Y-%m-%d"),
            "Signal_Type": "5-condition RSI trigger",
            **ret,
        })
...
def find_six_condition_events(data, symbol, membership):
    ...
    for trigger_date in trigger_dates:
        ...
        end_position = min(trigger_position + 5, len(data.index) - 1)

        future_dates = data.index[
            trigger_position:end_position + 1
        ]

        green_turn_dates = future_dates[
            data.loc[future_dates, "Daily_Supertrend_Green_Turn"]
        ]

        if len(green_turn_dates) == 0:
            continue

        green_date = green_turn_dates[0]
        green_key = pd.Timestamp(green_date).strftime("%Y-%m-%d")

        if green_key in already_alerted:
            continue

        # Membership is checked on the actual 6-condition alert date.
        if not is_member_on_date(membership, symbol, green_date):
            continue

        already_alerted.add(green_key)

        ret = add_forward_returns(data, green_date)

        days_after_rsi = data.index.get_loc(green_date) - trigger_position

        events.append({
            "Stock": symbol.upper(),
            "RSI_Trigger_Date": pd.Timestamp(trigger_date).strftime("%Y-%m-%d"),
            "Supertrend_Green_Date": green_key,
            "Days_After_RSI_Trigger": int(days_after_rsi),
            "Signal_Type": "6-condition RSI + Supertrend",
            **ret,
        })
...
def summarize(events, signal_type):
    if not events:
        return pd.DataFrame([{
    ...
    for h in [5, 10, 20, 60]:
        ...
        avg = valid.mean() if not valid.empty else None
        win = (valid > 0).mean() * 100 if not valid.empty else None

        rows.append((h, avg, win))

    return pd.DataFrame([{
        "Signal_Type": signal_type,
        "Events": len(df),
        "Avg_Return_5D_pct": rows[0][1],
        "Win_Rate_5D_pct": rows[0][2],
        "Avg_Return_10D_pct": rows[1][1],
        "Win_Rate_10D_pct": rows[1][2],
        "Avg_Return_20D_pct": rows[2][1],
        "Win_Rate_20D_pct": rows[2][2],
        "Avg_Return_60D_pct": rows[3][1],
        "Win_Rate_60D_pct": rows[3][2],
    }])


def summarize_by_stock(events):
    if not events:
        return pd.DataFrame()
    ...
    for stock, g in df.groupby("Stock"):
        ...
        for h in [5, 10, 20, 60]:
            ...
            row[f"Avg_Return_{h}D_pct"] = valid.mean() if not valid.empty else None
            row[f"Win_Rate_{h}D_pct"] = (
                (valid > 0).mean() * 100 if not valid.empty else None
            )

        rows.append(row)

    return pd.DataFrame(rows).sort_values(
        ["Avg_Return_20D_pct", "Events"],
...
def main():
    print()
    print("=" * 72)
    print("NIFTY 500 — 6-CONDITION SUPERTREND BACKTEST")
    print("=" * 72)
    print("Period:", START_DATE.strftime("%Y-%m-%d"), "to", END_DATE.strftime("%Y-%m-%d"))
    print()
    print("5-condition baseline:")
    print("  1. Monthly RSI(5) > SMA14")
    print("  2. Weekly RSI(5) > SMA14")
    print("  3. Monthly ADX(14) >= 25")
    print("  4. Daily RSI(5) <= 30")
    print("  5. Weekly ADX(14) >= 25")
    print()
    print("6th condition:")
    print("  Supertrend(10,1) must BECOME GREEN within 0-5 trading days")
    print("  after the RSI trigger. Entry/event = Supertrend green-turn close.")
    print()
    ...
    for start in range(0, total, BATCH_SIZE):
        ...
        for symbol in batch_symbols:
            try:
                ...
                if frame.empty:
                    # For MultiIndex extraction the symbol may be represented
                    # without the .NS suffix in some yfinance versions.
                ...
                if data.empty:
                    errors.append({
                        "Stock": symbol,
                        "Error": "Insufficient/invalid OHLC data"
                    })
                    continue

                five = find_five_condition_events(
                    data, symbol, membership
                )
                six = find_six_condition_events(
                    data, symbol, membership
                )

                all_five.extend(five)
                all_six.extend(six)

                print(
                    f"  {symbol}: 5-cond={len(five)}, "
                    f"6-cond={len(six)}"
                )
        ...
        # Small pause between Yahoo batches.
        time.sleep(1)

    five_df = pd.DataFrame(all_five)
    six_df = pd.DataFrame(all_six)
    ...
    if not six_df.empty:
        six_df.sort_values(
            ["Supertrend_Green_Date", "Stock"],
            inplace=True
        )

    five_df.to_csv(FIVE_EVENTS_FILE, index=False)
    six_df.to_csv(EVENTS_FILE, index=False)

    overall = pd.concat([
        summarize(all_five, "5-condition baseline"),
        summarize(all_six, "6-condition RSI + Supertrend"),
    ], ignore_index=True)

    overall.to_csv(SUMMARY_FILE, index=False)

    stock_summary = pd.concat([
        summarize_by_stock(all_five).assign(
            Signal_Type="5-condition baseline"
        ),
        summarize_by_stock(all_six).assign(
            Signal_Type="6-condition RSI + Supertrend"
        ),
    ], ignore_index=True)

    stock_summary.to_csv(
        "nifty500_6condition_by_stock.csv",
        index=False
    )
    ...
    print("=" * 72)
    print("5-condition events:", len(five_df))
    print("6-condition events:", len(six_df))
    ...
    print()
    print("Files created:")
    print(" ", FIVE_EVENTS_FILE)
    print(" ", EVENTS_FILE)
    print(" ", SUMMARY_FILE)
    print(" ", "nifty500_6condition_by_stock.csv")
    print(" ", ERRORS_FILE)
