...
def find_six_condition_events(data, symbol, membership):
    ...
    for trigger_date in trigger_dates:
        ...
        # 0..7 trading days, inclusive of the trigger day.
        end_position = min(trigger_position + 7, len(data.index) - 1)
        future_dates = data.index[trigger_position:end_position + 1]

        rsi_cross_date = None
        st_green_date = None

        for j in range(trigger_position, end_position + 1):
            ...
            # Supertrend only needs to be GREEN somewhere in the same window;
...
def main():
    ...
    with pd.ExcelWriter("nifty500_6condition_datewise.xlsx", engine="openpyxl") as writer:
        ...
        pd.DataFrame({"Criteria": [
            ...
            "5. Weekly ADX(14) >= 25",
            "6. After RSI <= 30, Daily RSI(5) crosses above Daily RSI SMA(14) within 0-7 trading days",
            "7. Daily Supertrend(10,1) is GREEN within the same 0-7 trading-day window",
